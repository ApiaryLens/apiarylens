...
| 2026-07-17 | Exact Product Preview 3 / installed Windows package and packaged HTTP backend | Installed-host security smoke plus interrupted standalone-to-connected migration, resume, reconciliation, cutover, and rollback | Pass | [`preview3-windows-packaged-migration-2026-07-17.json`](preview3-windows-packaged-migration-2026-07-17.json) | Installer SHA matched the immutable release; the exact installed `app.asar` migration/API/database/media modules transferred 527 records, one tombstone, and private media over authenticated HTTP in six bounded batches; an injected interruption resumed exactly once; full hashes and media reconciled; the journal contained no secret-like content; rollback restored standalone authority without modifying the owner's UAT environment. |
