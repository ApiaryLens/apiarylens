...
| Cross-family data access | Organization ID comes only from the authenticated session; every query and media key is scoped server-side | Negative resource, change, member, media, export, and database tests | Repeat conformance suite on both deployed profiles |
| Role escalation | Server-side permission map for membership, mutation, media, export, backup, and operator actions | Viewer and operator-boundary negative tests | Manual route inventory review |
